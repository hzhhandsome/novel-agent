...
- `ForeshadowingItem` 是正式结构化记忆的一部分，右侧模块栏只读取正式表，不直接读取候选任务快照。
- `judge_foreshadowing` 只产生候选判断；只有章节采纳或显式维护回填时，候选判断才会写入正式伏笔表。
- `new` 写入为 `planted`，`advanced` 更新为 `advanced`，`resolved` 更新为 `recovered`。同一项目内相同伏笔内容只保留一条，后续章节推进或回收时更新状态和备注。
- 模型可能返回字符串数组，也可能返回对象数组；写入前会优先读取 `content`、`伏笔`、`foreshadowing`、`description`、`text`、`summary` 字段，避免把整个 JSON 对象当作伏笔正文显示。
- `POST /api/projects/{project_id}/memory/backfill` 可从已采纳章节的 `chapter_generation` 任务快照回填正式伏笔表，用于修复旧数据；该动作不重新调用模型。
- 角色卡右侧模块栏应展示短内容：角色名、角色定位、当前目标、时期/阶段、时期摘要和关键关系。完整 JSON 判断和原始模型输出保留在 Agent 后台节点详情，不应塞满右侧模块栏。
