...
- 已完成第一阶段轻量混合检索：向量召回 + 关键词召回 union，按规则式 rerank 后进入上下文预算。
- Agent 后台能显示每条召回结果的 `retrieval_source`、`ranker`、匹配词和重排分。
- 当前未接外部或本地重排模型，重模型 reranker 保留为后续增强。
...
- 已增加轻量关键词召回：按 query term 匹配正式记忆正文和 metadata。
- 已与向量召回做 union，按 `source/source_id` 去重后保留来源和分数。
- 已实现规则式 rerank：
  - RAG 相似度。
  - 关键词命中。
  - 未回收伏笔优先。
